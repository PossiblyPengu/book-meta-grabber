<<<<<<< E:/CODE/book-meta-grabber/src/workers/ffmpegWorker.js
  ...
  if (cmd === 'concat') {
    try {
      ...
      for (let i = 0; i < parts.length; i++) {
        const p = parts[i];
        let data;
        if (p.file && typeof p.file.arrayBuffer === 'function') {
          data = await p.file.arrayBuffer();
        } else if (typeof p.arrayBuffer === 'function') {
          data = await p.arrayBuffer();
        } else {
          data = p.data;
        }
        
        const ext = p.ext || (p.name ? p.name.split('.').pop() : 'mp3');
...
export {};
>>>>>>> C:/Users/andre/.windsurf/worktrees/book-meta-grabber/book-meta-grabber-4fcc4624/src/workers/ffmpegWorker.js
