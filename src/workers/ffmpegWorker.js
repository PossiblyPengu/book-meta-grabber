...
self.addEventListener('message', async (ev) => {
  ...
  if (cmd === 'abort') {
    aborted = true;
    try { ffmpeg?.exit && ffmpeg.exit(); } catch {}
  ...
  if (cmd === 'concat') {
    try {
      ...
      for (let i = 0; i < parts.length; i++) {
        const p = parts[i];
        const data = p.arrayBuffer instanceof Function ? await p.arrayBuffer() : p.data;
      ...
      for (let i = 0; i < parts.length; i++) try { ffmpeg.FS('unlink', `part${i}.${parts[i].ext || 'mp3'}`); } catch {}
