<<<<<<< E:/CODE/book-meta-grabber/src/workers/metadataWorker.js
  ...
  if (cmd === 'parse') {
    try {
      ...
      const result = {
        ...
        publisher: common.label?.[0] || '',
        description: common.comment?.[0] || '',
        genre: isAudiobook(common, fileName, format.duration) ? 'Audiobook' : (common.genre?.[0] || ''),
        language: common.language || '',
        coverBase64,
        coverMime,
        duration: format.duration ? Math.round(format.duration) : null,
...
function findNarrator(common) {
  if (common.comment) {
    ...
    const narratorComment = comments.find(c => 
      c.toLowerCase().includes('narrator') ||
      c.toLowerCase().includes('read by') ||
      c.toLowerCase().includes('voiced by')
    );
    if (narratorComment) {
      const match = narratorComment.match(/(?:narrator|read by|voiced by)[:\s]+(.+?)(?:[,;]|$)/i);
...
export {};
>>>>>>> C:/Users/andre/.windsurf/worktrees/book-meta-grabber/book-meta-grabber-4fcc4624/src/workers/metadataWorker.js
