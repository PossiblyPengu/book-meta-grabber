...
export function SearchView(results = [], loading = false) {
  const resultCards = results
    .map(
      (r) => `
      <div class="search-result-card" data-action="add-search-result" data-index="${results.indexOf(r)}">
        <div class="search-result-cover">
          ${r.coverUrl ? `<img src="${escapeHtml(r.coverUrl)}" alt="" loading="lazy">` : ''}
        </div>
  ...
  return `
    <div class="search-view">
      ...
      <div class="search-bar">
        ...
        <button class="btn btn-primary" data-action="api-search">${icons.search} Search</button>
      </div>
